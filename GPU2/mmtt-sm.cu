...
// Enter GPU kernel code body
    ...
    __shared__ double Abuffer[SHAREDSIZE * SHAREDSIZE];
    __shared__ double Bbuffer[SHAREDSIZE * SHAREDSIZE];
    ...
    if (col < ds && row < ds)
    {
        ...
        for (int kp = 0; kp < DSIZE/ SharedSize;kp++)
        {
            ...
            Abuffer[(ty * SharedSize) + tx] = A[(kp* SharedSize *ds)+(ty*ds)+row];
            Bbuffer[(ty* SharedSize)+tx] = B[(col*ds)+(kp* SharedSize)+tx];
            __syncthreads();
            //Due calc on the subset save to temp
            for (int k = 0;k < SharedSize;k++)
            {
                sum += Bbuffer[ty* SharedSize +k] * Abuffer[k* SharedSize +tx];
            }
            __syncthreads();
        }
        C[row * ds + col] = sum;
...
int main(){
  ...
  while(1)
 {
  ...
  block.x = Bx;
  block.y = By;
  grid.x = ceil(DSIZE/block.x);
  grid.y = ceil(DSIZE/block.y);
