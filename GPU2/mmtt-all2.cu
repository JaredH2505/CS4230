...
// Enter GPU kernel code body
    ...
    int i2 = (blockDim.y * (by + gridDim.y)) + ty;
    int j2 = (blockDim.x * (bx + gridDim.x)) + tx;
    ...
    if (i < (ds/2) && j < (ds/2))
    {
        ...
        {
            sumij += A[(k * ds) + i] * B[(j * ds) + k];
            sumij += A[((k + 1) * ds) + i] * B[(j * ds) + k + 1];

            sumij2 += A[(k * ds) + i] * B[(j2 * ds) + k];
            sumij2 += A[((k + 1) * ds) + i] * B[(j2 * ds) + k + 1];

            sumi2j += A[(k * ds) + i2] * B[(j * ds) + k];
            sumi2j += A[((k + 1) * ds) + i2] * B[(j * ds) + k + 1];

            sumi2j2 += A[(k * ds) + i2] * B[(j2 * ds) + k];
            sumi2j2 += A[((k + 1) * ds) + i2] * B[(j2 * ds) + k + 1];
        }
        C[i * ds + j] = sumij;
        C[i2 * ds + j] = sumi2j;
        C[i * ds + j2] = sumij2;
