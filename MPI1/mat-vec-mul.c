...
// FIXME: Initially identical to reference; make your changes to parallelize this code
{
  ...
  int iStart = chunkSize * myid;
  int iEnd = chunkSize * (myid+1);

  for (i = 0; i < n; i++)
  {
    y[i] = 0.0;
  }
  //Divide y into p parts and only calc our part of p.
  //Then use broadcast or gather all to join all parts for the next run.
  for (iter = 0; iter < Niter; iter++)
  {
    //Find i start and end
    ...
    for (i = iStart; i < iEnd; i++)
    {
      ...
      for (j = 0; j < n; j++)
      {
        ...
        //Send my part to the others
        //Join all others to me before continuing.
        MPI_Allgather(MPI_IN_PLACE, chunkSize, MPI_DOUBLE, &y[0], chunkSize, MPI_DOUBLE, MPI_COMM_WORLD);
